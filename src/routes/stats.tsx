        ...
        {Array.from({ length: totalDays }, (_, i) => {
          ...
          return (
            <button
              ...
              className={`group relative min-h-12 overflow-hidden rounded-xl border px-2 py-1.5 text-left transition sm:min-h-14 sm:rounded-2xl ${selected ? 'border-accent ring-2 ring-accent/25' : 'border-transparent hover:border-border'}`}
            >
              ...
              <div className="relative flex h-full min-h-9 flex-col justify-between">
                ...
                {info ? <span className="truncate text-[9px] font-medium text-muted sm:text-[10px]">{duration(info.seconds)}</span> : <span className="text-[9px] text-faint">·</span>}
              </div>
            </button>
          );
        })}
...
function DayGames({ rows, selectedDate }: { rows: WebActivity['daily']; selectedDate: string }) {
...
function Timeline({ monthCursor, setMonthCursor, month, byDay, selectedDate, setSelectedDate, daily }: {
  monthCursor: Date; setMonthCursor: (date: Date) => void; month: string;
  byDay: Map<string, { seconds: number; sessions: number; top: WebActivityGame | null }>;
  selectedDate: string; setSelectedDate: (date: string) => void; daily: WebActivity['daily'];
}) {
  return (
    <section className="rounded-[2rem] bg-elevated p-4 sm:p-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(280px,0.95fr)] lg:items-stretch">
        ...
        <div className="rounded-3xl bg-subtle/45 p-4 sm:p-5"><DayGames rows={daily} selectedDate={selectedDate} /></div>
      </div>
    </section>
  );
}
...
function StatsPage() {
  ...
  const activity = useQuery({ queryKey: ['play-history', month], queryFn: ({ signal }) => getActivity(signal, month), enabled: Boolean(user), staleTime: 2 * 60_000 });

  useEffect(() => {
    if (!selectedDate.startsWith(month)) setSelectedDate(month === monthKey(new Date()) ? dateKey(new Date()) : `${month}-01`);
  }, [month, selectedDate]);

  if (isPending || library.isLoading) return <div className="space-y-5"><Skeleton className="h-48 w-full rounded-3xl" /><Skeleton className="h-52 w-full rounded-2xl" /><Skeleton className="h-64 w-full rounded-2xl" /></div>;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-12">
      ...
      <Timeline monthCursor={monthCursor} setMonthCursor={setMonthCursor} month={month} byDay={byDay} selectedDate={selectedDate} setSelectedDate={setSelectedDate} daily={daily} />
      ...
      {activity.error ? <p className="text-center text-xs text-muted">Activity data is temporarily unavailable.</p> : null}
    </div>
  );
}
