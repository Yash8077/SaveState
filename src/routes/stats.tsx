...
function ArtCard({ game, rank, totalSeconds }: { game: WebActivityGame; rank: number; totalSeconds: number }) {
  const share = totalSeconds > 0 ? Math.max(0.08, Math.min(1, game.seconds / totalSeconds)) : 0;
  return (
    <button
      type="button"
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-3">
        ...
        <div className="mt-2 flex items-center justify-between text-[11px] text-white/75"><span>{duration(game.seconds)}</span><span>{game.sessions} sessions</span></div>
        <div className="mt-2 h-1 overflow-hidden rounded-full bg-white/20"><div className="h-full rounded-full bg-white" style={{ width: `${share * 100}%` }} /></div>
      </div>
    </button>
  );
}
...
function Calendar({
  ...
  return (
    <div>
      <div className="mb-2 grid grid-cols-7 gap-1 text-center text-[9px] font-bold uppercase tracking-[0.16em] text-faint sm:gap-1.5">
        {["M", "T", "W", "T", "F", "S", "S"].map((day, i) => <div key={`${day}-${i}`}>{day}</div>)}
      </div>
      <div className="grid grid-cols-7 gap-1 sm:gap-1.5">
        {Array.from({ length: leading }).map((_, i) => <div key={`empty-${i}`} className="min-h-10 sm:min-h-11" aria-hidden="true" />)}
        {Array.from({ length: totalDays }, (_, i) => {
          ...
          return (
            <button
              ...
              className={[
                "group relative min-h-10 overflow-hidden rounded-xl border px-1.5 py-1 text-left transition sm:min-h-11 sm:rounded-2xl",
                selected ? "border-accent bg-accent/8 ring-2 ring-accent/20" : "border-transparent bg-subtle hover:border-border hover:-translate-y-px",
              ].join(" ")}
            >
              {info?.top?.coverUrl ? <img src={info.top.coverUrl} alt="" referrerPolicy="no-referrer" className="absolute inset-0 size-full object-cover" style={{ opacity: 0.08 + intensity * 0.62 }} /> : null}
              <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent" />
              <div className="absolute inset-x-1.5 bottom-1 h-0.5 overflow-hidden rounded-full bg-accent/15">
                <div className="h-full rounded-full bg-accent" style={{ width: `${info ? intensity * 100 : 0}%` }} />
              </div>
              <span className={["relative text-[11px] font-bold sm:text-xs", selected ? "text-accent" : "text-fg"].join(" ")}>{day}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    <div className="flex min-h-0 flex-col">
      <div className="mb-3 flex items-end justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-faint">Selected day</p>
          <h3 className="mt-1 text-base font-semibold sm:text-lg">{prettyDay(selectedDate)}</h3>
        </div>
      ...
      {loading ? (
        ...
        <div className="min-h-0 space-y-2 overflow-y-auto pr-1 lg:max-h-[300px]">
          {selectedRows.map((row) => (
            <button key={`${row.date}-${row.titleId}`} type="button" onClick={() => openGame(row)} className="group flex w-full items-center gap-3 rounded-2xl bg-subtle p-2.5 text-left transition hover:bg-border">
              {row.coverUrl ? <img src={row.coverUrl} alt="" referrerPolicy="no-referrer" className="size-11 shrink-0 rounded-xl object-cover transition group-hover:scale-[1.03]" /> : <div className="size-11 shrink-0 rounded-xl bg-accent/10" />}
...
function Timeline({ monthCursor, setMonthCursor, month, byDay, selectedDate, setSelectedDate, daily, loading }: {
  ...
  daily: WebActivity["daily"];
  loading: boolean;
}) {
  return (
    <section className="relative overflow-hidden rounded-[2rem] bg-elevated p-4 sm:p-5">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-faint">Activity calendar</p><h2 className="mt-1 text-lg font-semibold sm:text-xl">{monthLabel(monthCursor)}</h2></div>
        <div className="flex items-center gap-1.5">
          <button type="button" aria-label="Previous month" onClick={() => setMonthCursor(new Date(monthCursor.getFullYear(), monthCursor.getMonth() - 1, 1))} className="grid size-9 place-items-center rounded-full bg-subtle text-base font-semibold text-muted transition hover:bg-border hover:text-fg">‹</button>
          <button type="button" aria-label="Next month" onClick={() => setMonthCursor(new Date(monthCursor.getFullYear(), monthCursor.getMonth() + 1, 1))} className="grid size-9 place-items-center rounded-full bg-subtle text-base font-semibold text-muted transition hover:bg-border hover:text-fg">›</button>
        </div>
      </div>
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.92fr)] lg:items-stretch">
        <div className="rounded-3xl bg-subtle/60 p-3 sm:p-4"><Calendar monthCursor={monthCursor} month={month} byDay={byDay} selectedDate={selectedDate} onSelect={setSelectedDate} /></div>
        <div className="rounded-3xl bg-subtle/35 p-3 sm:p-4"><DayGames rows={daily} selectedDate={selectedDate} loading={loading} /></div>
      </div>
      {loading ? <div className="pointer-events-none absolute right-5 top-5 flex items-center gap-2 rounded-full bg-elevated/90 px-3 py-1.5 text-[10px] font-semibold text-muted shadow-sm backdrop-blur"><span className="size-2 animate-pulse rounded-full bg-accent" /> Updating month</div> : null}
    </section>
  );
}
...
function StatsPage() {
  ...
  const summaryActivity = useQuery({
    queryKey: ["play-history-summary"],
  ...
  const monthActivity = useQuery({
    queryKey: ["play-history-month", month],
  ...
  useEffect(() => {
    if (!selectedDate.startsWith(month)) {
      setSelectedDate(month === monthKey(new Date()) ? dateKey(new Date()) : `${month}-01`);
    }
  }, [month, selectedDate]);

  if (isPending || library.isLoading || summaryActivity.isPending) {
    return <div className="space-y-5"><Skeleton className="h-44 w-full rounded-3xl" /><Skeleton className="h-48 w-full rounded-2xl" /><Skeleton className="h-64 w-full rounded-2xl" /></div>;
  }
  ...
  for (const row of daily) {
    ...
    if (!existing.top || row.seconds > existing.top.seconds) existing.top = game ? { ...game, seconds: row.seconds } : { titleId: row.titleId, titleName: row.titleName, seconds: row.seconds, sessions: row.sessions, lastPlayed: row.date, platform: row.platform, libraryGameId: null, catalogId: row.catalogId, coverUrl: row.coverUrl, headerUrl: row.headerUrl };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-7 pb-12">
      ...
      <section><div className="mb-3"><p className="text-xs font-medium uppercase tracking-[0.18em] text-faint">Your rotation</p><h2 className="mt-1 text-lg font-medium">Most played</h2></div>{games.length ? <div className="rail-scroll gap-3">{games.slice(0, 8).map((game, index) => <ArtCard key={game.titleId} game={game} rank={index + 1} totalSeconds={totals.seconds} />)}</div> : <div className="rounded-2xl bg-subtle p-8 text-center text-sm text-muted">Run the SaveState PS5 activity logger after a session to build this timeline.</div>}</section>
