...
class AuthUser {
  ...
  final String name;
  final String? image;
  AuthUser({required this.id, required this.email, required this.name, this.image});
  AuthUser copyWith({String? name, String? image}) => AuthUser(id: id, email: email, name: name ?? this.name, image: image ?? this.image);
  Map<String, dynamic> toJson() => {'id': id, 'email': email, 'name': name, 'image': image};
  factory AuthUser.fromJson(Map<String, dynamic> json) => AuthUser(id: json['id']?.toString() ?? '', email: json['email']?.toString() ?? '', name: json['name']?.toString() ?? '', image: json['image'] as String?);
}
...
class ApiClient {
  ...
  Future<void> hydrate() async {
    ...
    if (featuredRaw != null) { try { final decoded = jsonDecode(featuredRaw); if (decoded is List) { _featuredCache = [for (final row in decoded) if (row is Map) FeaturedRail.fromJson(Map<String, dynamic>.from(row))]; _featuredAt = DateTime.now().subtract(const Duration(days: 1)); } } catch (_) {} }
  ...
  Future<dynamic> _send(String method, Uri uri, {Object? body, bool jsonBody = false}) async {
    final headers = _headers(json: jsonBody); late final http.Response res;
    try { switch (method) { case 'GET': res = await _client.get(uri, headers: headers).timeout(_httpTimeout); break; case 'POST': res = await _client.post(uri, headers: headers, body: jsonBody ? jsonEncode(body) : body).timeout(_httpTimeout); break; case 'PATCH': res = await _client.patch(uri, headers: headers, body: jsonEncode(body)).timeout(_httpTimeout); break; case 'DELETE': res = await _client.delete(uri, headers: headers).timeout(_httpTimeout); break; default: throw ApiException(0, 'Unsupported $method'); } } on TimeoutException { throw ApiException(0, 'Request timed out'); }
    if (res.statusCode >= 400) { String message = 'HTTP ${res.statusCode}'; try { final decoded = jsonDecode(res.body); if (decoded is Map && decoded['message'] is String) message = decoded['message'] as String; else if (decoded is Map && decoded['error'] is String) message = decoded['error'] as String; } catch (_) { if (res.body.trim().isNotEmpty) message = res.body; } throw ApiException(res.statusCode, message); }
    if (res.body.isEmpty) return null; return jsonDecode(res.body);
  }
  ...
  Future<void> deleteEntry(int id) async { await _send('DELETE', _u('/api/library/$id')); _invalidateLibrary(); }
  Future<Map<String, dynamic>> exportBackup() async { final decoded = await _send('GET', _u('/api/backup')); if (decoded is Map<String, dynamic>) return decoded; throw ApiException(500, 'Invalid backup'); }
  Future<({int added, int updated})> importBackup(Object body) async { final decoded = await _send('POST', _u('/api/backup'), jsonBody: true, body: body); _invalidateLibrary(); if (decoded is! Map) throw ApiException(500, 'Invalid import'); return (added: (decoded['added'] as num?)?.toInt() ?? 0, updated: (decoded['updated'] as num?)?.toInt() ?? 0); }
  Future<bool> googleAuthEnabled() async { try { final decoded = await _send('GET', _u('/api/config')); return decoded is Map && decoded['google'] == true; } catch (_) { return false; } }
  Future<List<String>> listAvatars() async { try { final decoded = await _send('GET', _u('/api/config')); if (decoded is! Map) return const []; final raw = decoded['avatars']; if (raw is! List) return const []; return raw.map((e) => e.toString()).where((s) => s.startsWith('/avatars/avatar_')).toList(); } catch (_) { return const []; } }
  Future<({AuthUser user, String token})> signInEmail(String email, String password) async => _authPost('/api/auth/sign-in/email', {'email': email.trim(), 'password': password});
  Future<({AuthUser user, String token})> signUpEmail({required String email, required String password, required String name}) async => _authPost('/api/auth/sign-up/email', {'email': email.trim(), 'password': password, 'name': name.trim().isEmpty ? email.split('@').first : name.trim()});
  Future<({AuthUser user, String token})> _authPost(String path, Map<String, String> body) async { final uri = _u(path); final res = await _client.post(uri, headers: _headers(json: true), body: jsonEncode(body)); dynamic decoded; try { decoded = res.body.isEmpty ? null : jsonDecode(res.body); } catch (_) {} if (res.statusCode >= 400) { final msg = decoded is Map ? (decoded['message'] ?? decoded['error'] ?? 'Auth failed') : 'Auth failed (${res.statusCode})'; throw ApiException(res.statusCode, msg.toString()); } final token = _tokenFrom(decoded, res); if (token == null) throw ApiException(res.statusCode, 'Signed in but no session token'); final userMap = decoded is Map ? decoded['user'] : null; if (userMap is! Map) throw ApiException(res.statusCode, 'Signed in but no user'); return (user: AuthUser(id: userMap['id']?.toString() ?? '', email: userMap['email']?.toString() ?? '', name: userMap['name']?.toString() ?? '', image: userMap['image']?.toString()), token: token); }
  Future<AuthUser?> getSession() async { if (sessionToken == null) return null; try { final decoded = await _send('GET', _u('/api/auth/get-session')); if (decoded is Map && decoded['user'] is Map) { final u = decoded['user'] as Map; return AuthUser(id: u['id']?.toString() ?? '', email: u['email']?.toString() ?? '', name: u['name']?.toString() ?? '', image: u['image']?.toString()); } } on ApiException catch (e) { if (e.status == 401) return null; rethrow; } return null; }
  Future<void> signOut() async { try { await _send('POST', _u('/api/auth/sign-out'), jsonBody: true, body: {}); } catch (_) {} }
  Future<Map<String, dynamic>> getProfile() async { final decoded = await _send('GET', _u('/api/profile')); if (decoded is Map<String, dynamic>) return decoded; if (decoded is Map) return Map<String, dynamic>.from(decoded); throw ApiException(500, 'Invalid profile'); }
  Future<Map<String, dynamic>> updateProfile({String? name, String? image, String? banner, int? bannerY, bool clearBanner = false}) async { final decoded = await _send('PATCH', _u('/api/profile'), jsonBody: true, body: {if (name != null) 'name': name, if (image != null) 'image': image, if (clearBanner) 'banner': null, if (!clearBanner && banner != null) 'banner': banner, if (bannerY != null) 'bannerY': bannerY}); if (decoded is Map<String, dynamic>) return decoded; if (decoded is Map) return Map<String, dynamic>.from(decoded); throw ApiException(500, 'Invalid profile'); }
  Future<void> changePassword({required String currentPassword, required String newPassword}) async { await _send('POST', _u('/api/profile/password'), jsonBody: true, body: {'currentPassword': currentPassword, 'newPassword': newPassword}); }
}
