//#region node_modules/.nitro/vite/services/ssr/assets/catalog-seed-GyIvdhCE.js
...
var SEED_GAMES = RAW.map(([id, title]) => toGame(id, title));
var INDEX = new Map(SEED_GAMES.map((game) => [game.steamId, game]));
function pick(ids) {
	const games = [];
	for (const id of ids) {
...
function searchSeed(query) {
	...
	for (const game of SEED_GAMES) {
		...
		if (!title.includes(q) && !q.split(" ").every((part) => title.includes(part))) continue;
		let score = 0;
...
function mergeFeaturedRails(live) {
	if (!live.length) return FEATURED_SEED;
	if ((live[0]?.games[0]?.id ?? "").startsWith("igdb_")) return live;
	return [FEATURED_SEED[0], ...live.filter((rail) => rail.id !== "top_sellers" && rail.id !== "popular")];
}
//#endregion
export { slimCatalogGame as i, mergeFeaturedRails as n, searchSeed as r, FEATURED_SEED as t };
